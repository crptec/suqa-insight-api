...
AddressController.prototype.getAddressSummary = function(address, options, callback) {
  //node -> services -> suqacored -> getAddressSummary
  this.node.services.suqacored.getAddressSummary(address, options, function(err, summary) {
	console.log(err);
    if(err) {
      return callback(err);
    }
...
AddressController.prototype.check = function(req, res, next, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    try {
      var a = new suqacore.Address(addresses[i]);
    } catch(e) {
      return self.common.handleErrors({
...
AddressController.prototype.transformUtxo = function(utxoArg) {
  ...
  if (utxoArg.height && utxoArg.height > 0) {
    ...
    utxo.confirmations = this.node.services.suqacored.height - utxoArg.height + 1;
  } else {
