...
var suqacore = require('suqacore-lib');
var _ = suqacore.deps._;
var pools = require('../pools.json');
var BN = suqacore.crypto.BN;
...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.suqacored.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.suqacored.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.suqacored.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.suqacored.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.suqacored.height - blockCached.height + 1;
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      //self.node.services.suqacored.getBlockHeader(hash, function(err, info) {
		self.node.services.suqacored.getBlock(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix suqacore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.suqacored.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.suqacored.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new suqacore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in suqacore-node.
      var header = suqacore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [suqacore.Transaction().fromBufferReader(br)];

      self.node.services.suqacored.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.suqacored.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.suqacored.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.suqacored.getBlockHeader(prevHash, function (err, info) {
              if (err) {
